...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	if ((a.usePenalty && b.isWall) || (b.usePenalty && a.isWall)) {
		const float constant = 10.0f;

		Vector3 action;
		Vector3 reaction;

		Vector3 forceDir = b.GetTransform().GetPosition() - a.GetTransform().GetPosition();

		Vector3 extension = forceDir.Normalised() * sqrt(p.penetration * p.penetration);

		action = extension * constant;
		reaction = -extension * constant;

		a.GetPhysicsObject()->ApplyLinearImpulse(reaction);
		b.GetPhysicsObject()->ApplyLinearImpulse(action);
