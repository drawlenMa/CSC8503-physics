...
bool CollisionDetection::AABBIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
    ...
    bool overlap = AABBTest(boxAPos, boxBPos, boxASize, boxBSize); if (overlap) {
...
bool  CollisionDetection::OBBSphereIntersection(const OBBVolume& volumeA, const Transform& TransformA,
	const SphereVolume& volumeB, const Transform& TransformB, CollisionInfo& collisionInfo) {
	Transform Firsttransform;
	Transform Secondtransform;
	AABBVolume Volume(volumeA.GetHalfDimensions());
	const Vector3 delta = TransformB.GetPosition() - TransformA.GetPosition();
	Secondtransform.SetPosition(TransformA.GetOrientation().Conjugate() * delta);
	CollisionInfo& colIn = collisionInfo;
	if (AABBSphereIntersection(Volume, Firsttransform, volumeB, Secondtransform, collisionInfo))
	{
		colIn.point.localA = TransformA.GetOrientation() * colIn.point.localA;
		colIn.point.localB = TransformA.GetOrientation() * colIn.point.localB;
		colIn.point.normal = TransformA.GetOrientation() * colIn.point.normal;
		return true; 
	}
	return false; 
}
